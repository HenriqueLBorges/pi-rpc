...
class PiService$FinagleService(
  ...
  ) = this(iface, protocolFactory, stats, maxThriftBufferSize, "PiService")
  ...
  addService("getNumber", {
    val methodService = new _root_.com.twitter.finagle.Service[GetNumber.Args, GetNumber.SuccessType] {
      def apply(args: GetNumber.Args): Future[GetNumber.SuccessType] = {
        if (_root_.com.twitter.finagle.tracing.Trace.isActivelyTracing) {
          _root_.com.twitter.finagle.tracing.Trace.recordRpc("getNumber")
        }
        iface.getNumber()
      }
    }
  
    filters.getNumber.andThen(methodService)
  })
  addService("submitNumber", {
    val methodService = new _root_.com.twitter.finagle.Service[SubmitNumber.Args, SubmitNumber.SuccessType] {
      def apply(args: SubmitNumber.Args): Future[SubmitNumber.SuccessType] = {
        if (_root_.com.twitter.finagle.tracing.Trace.isActivelyTracing) {
          _root_.com.twitter.finagle.tracing.Trace.recordRpc("submitNumber")
        }
        iface.submitNumber(args.result)
      }
    }
  
    filters.submitNumber.andThen(methodService)
  })
  addService("getPI", {
    val methodService = new _root_.com.twitter.finagle.Service[GetPI.Args, GetPI.SuccessType] {
      def apply(args: GetPI.Args): Future[GetPI.SuccessType] = {
        if (_root_.com.twitter.finagle.tracing.Trace.isActivelyTracing) {
          _root_.com.twitter.finagle.tracing.Trace.recordRpc("getPI")
        }
        iface.getPI()
      }
    }
  
    filters.getPI.andThen(methodService)
  })
}